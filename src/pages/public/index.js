...
function Home(){
    ...
    async function xemTiepBds() {
        ...
        setCurrentPage(cur);
        var response = await getMethod('/api/real-estate/public/get-full?size='+sizepro+'&sort=id,desc'+'&page='+cur);
        var result = await response.json();
            ...
            <h3>Bất động sản dành cho bạn</h3>
                ...
                {realEstate.map((item=>{
                    return <div class="col-sm-3">
                        <div class="singlebds">
                            <a href={'chi-tiet-tin-dang?id='+item.id}><img src={item.image} class="imgdbs"/></a>
                            <div class="contentbds">
                                <a href={'chi-tiet-tin-dang?id='+item.id} class="titlebdssingle">{item.title}</a>
                                <div class="divprice">
                                    <span class="pricebds">{formatPrice(item.price)}</span>
                                    <span class="pricebds areabds">{item.acreage} m<sup>2</sup> </span>
                                </div>
                                <span class="diachibds"><i class="fa fa-map-marker"></i> {item.ward.name}, {item.ward.district.name}<br/>{item.ward.district.province.name}</span>
                                <div class="divtimebds">
                                    <span class="timedangbds">{formatDate(item.createdDate)}</span>
