...
function TimKiem(){
    ...
    useEffect(()=>{
        ...
        const getBds = async() =>{
            var payload = {
                ...
                provinceId: objSearch.tinh.value==0?null:objSearch.tinh.value,
            ...
            var response = await postMethodPayload('/api/real-estate/public/search-full-accuracy?size='+sizepro+'&sort=id,desc'+'&page='+0, payload);
            var result = await response.json();
            setTotalResult(result.totalElements);
            setpageCount(result.totalPages)
            setRealEstate(result.content)
            url = '/api/real-estate/public/search-full-accuracy?size='+sizepro+'&sort=id,desc'+'&page='
        };
        getBds();
    }, []);
    ...
    async function locTinXacThuc() {
        ...
        var response = await postMethodPayload('/api/real-estate/public/search-full-accuracy?size='+sizepro+'&sort='+sort+'&page='+0, payload);
        var result = await response.json();
        setTotalResult(result.totalElements);
        setpageCount(result.totalPages)
        setRealEstate(result.content)
        setCurrentPage(0);
        url = '/api/real-estate/public/search-full-accuracy?size='+sizepro+'&sort='+sort+'&page='
    }

    async function sapXepTin() {
        var sort = document.getElementById("sortby").value;
        var response = await postMethodPayload('/api/real-estate/public/search-full-accuracy?size='+sizepro+'&sort='+sort+'&page='+0, payloadSearch);
        var result = await response.json();
        setTotalResult(result.totalElements);
        setpageCount(result.totalPages)
        setRealEstate(result.content)
                    ...
                    <div class="col-sm-9">
                        <div class="filterdstimkiem row">
                            ...
                            <div class="col-sm-4 infiltertimkiem">
                                <select onChange={sapXepTin} class="form-control" id="sortby">
                                    <option value="id,desc">Tin mới nhất</option>
                                    <option value="id,asc">Tin cũ nhất</option>
                                    <option value="price,asc">Giá thấp đến cao</option>
                                    <option value="price,desc">Giá cao đến thấp</option>
                                </select>
                            </div>
                            <div className='col-sm-3 infiltertimkiem'>
                                <div class="divdaxacnhan" onclick="document.getElementById('checkxacthuc').click()">
                                    <span class="spxacnhan">
                                        <span class="tieudext">Tin xác thực</span>
                                        <label class="switch">
                                            <input onChange={locTinXacThuc} type="checkbox" id="checkxacthuc"/>
                        ...
                        {realEstate.map((item=>{
                            return <div class="singledbstimkiem">
                                <a href={'chi-tiet-tin-dang?id='+item.id}><div class="row">
                                    <div class="col-sm-8 nopaddingright">
                                        <img src={item.image} class="bdsimagelarge"/>
                                    </div>
                                    <div class="col-sm-4 nopaddingleft">
                                        <div class="row">
                                            <div class="col-sm-12 nopaddingbottom">
                                                <img src={item.realEstateImages[1]?.image} class="bdsimgsmall smallfirst"/>
                                            </div>  
                                            <div class="col-sm-6 nopaddingright">
                                                <img src={item.realEstateImages[2]?.image} class="bdsimgsmall smallsv"/>
                                            </div>
                                            <div class="col-sm-6 nopaddingleft divslanh">
                                                <img src={item.realEstateImages[3]?.image} class="bdsimgsmall smallsv"/>
                                                <div class="soluonganh">
                                                    <i class="fa fa-picture-o"></i> {item.realEstateImages.length}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="contentbdstk">
                                    <a href={'chi-tiet-tin-dang?id='+item.id} class="tieudetk">{item.accuracy == false?'':<span class='daxacthucsp'>Đã xác thực</span>} {item.title}</a>
                                    <div class="divprice divtienich">
                                        <span class="pricebds">{formatPrice(item.price)}</span>
                                        <span class="pricebds areabds">{item.acreage} m<sup>2</sup></span>
                                        <span class="tienichtk">{formatPriceLT(Number(item.price / item.acreage).toString().split(".")[0])}/m²</span>
                                        <span class="tienichtk">{item.roomNumber} <i class="fa fa-bed"></i></span>
                                        <span class="tienichtk">{item.toiletNumber} <i class="fa fa-bathtub"></i></span>
                                        <span class="tienichtk">{item.ward.district.name}, {item.ward.district.province.name}</span>
                                        <span class="tienichtk ngaydangtk">{item.createdDate.split("T")[0]}</span>
                                    </div>
                                    <span class="motatkbds" dangerouslySetInnerHTML={{__html:item.description}}></span>
                                </div>
                                <hr/>
                                <div class="divtimebds">
                                    <img src={item.user.avatar == null?avatar:item.user.avatar} class="usertkiem"/>
                                    <span class="tenusertk">{item.user.fullname == null? "Người dùng": item.user.fullname }</span>
                                    <button onClick={()=>window.location.href='tel:'+item.user.phone} class="btnyeuthich iconyttk">0918 606 666</button>
                                </div>
                            </div>
                        }))}
                        </div>
                        ...
                        <div class="xemthembds">
                            <button onClick={xemTiepBds} class="btnxemthem">Mở rộng <i class="fa fa-chevron-down"></i></button>
                        </div>
                    </div>
                    <div class="col-sm-3">
                        <div class="locnhanhtk">
                            ...
                            <div id="listgiafilter">
                                {priceRange.map((item=>{
                                    return <a href={'timkiem?mucgia='+item.price}>{item.name}</a>
                                }))}
                            </div>
                        </div>
                        <div class="locnhanhtk">
                            ...
                            <div id="listdtbenphai">
                                {acreageRange.map((item=>{
                                    return <a href={'timkiem?dientich='+item.dientich}>{item.name}</a>
                                }))}
                            </div>
                        </div>
                    </div>
